...
export default function ProductPage({ locale, dictionary }: ProductPageProps) {
  ...
  return <main className="product-detail">
    ...
    <nav className="pd-breadcrumb" aria-label="Breadcrumb"><Link href={root}>{common.nav.home}</Link><span>/</span><Link href={localePath(locale, "/products")}>{common.nav.products}</Link><span>/</span><span>{product.options.specifications[0][1]}</span></nav>
